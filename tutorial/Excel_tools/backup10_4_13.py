    ...
    for i in range(nrows):
        if table.cell(i,column).value=='姓名' or table.cell(i,column).value=='序号':
            ...
            if i+2<nrows:
                print('多行数据，无法打印')
                return
        print(table.cell(i,column).value)
    #读第一个文件，先写一个模板
...
#创建图形化界面
window = tk.Tk()
window.title('Excel 统计工具        贾良军出品')
window.geometry('400x400')

tk.Label(window,font=('Arial', 10),text='输入 姓名 或 序号 (将会以此类别为依据进行查找)',anchor='w').place(x=10,y=10)
ChooseCategory=tk.StringVar()
ChooseCategory.set('姓名')
tk.Entry(window,textvariable=ChooseCategory,width=10).place(x=10,y=30)
...
gener=tk.Button(window, text='开始统计', width=15, command=Generate).place(x=10,y=130)
